...
public class APTrustHelper extends TarBagHelper {
        ...
        public enum Access {
                Consortia, Restricted, Institution;
        }
        ...
        public APTrustHelper(File parent, boolean allowSourceRename) {
                this(parent, allowSourceRename, true);
        }
        public APTrustHelper(File parent, boolean allowSourceRename, boolean makeTar) {
                super(parent, makeTar);
        ...
        public void setInstitutionalSenderDesc(String intSendDesc) {
                // bagit seems to have an issue with trailing periods in this
                // field
        ...
        public void validateImpl(StringBuilder sb) throws IncompleteSettingsException {
                ...
                // if (intSendDesc.isEmpty()) sb.append("Institution Sender
        ...
        public void generateBagInfoFiles() throws IOException, IncompleteSettingsException {
                validate();
                if (data.newBag == null)
                        throw new IncompleteSettingsException("Bag File must be created - call createBagFile()");
        ...
        public void writeBagFile() throws IOException, IncompleteSettingsException {
                if (aptinfo == null)
                        throw new IncompleteSettingsException(
                                        "Aptinfo File must be created - call generateBagInfoFiles()");
                super.writeBagFile();
                aptinfo.delete();
        }
        ...
        public void parseMetsFile(File zeout) throws IOException, InvalidMetadataException {
                try {
                        ...
                        if (id == null)
                                throw new InvalidMetadataException("mets.xml root element must have an OBJID field");
                        if (id.isEmpty())
                                throw new InvalidMetadataException(
                                                "mets.xml root element must not have an empty OBJID field");
                        ...
                        String parent = xp.evaluate(
                                        "/mets:mets/mets:structMap[@LABEL='Parent']/mets:div[@TYPE='AIP Parent Link']/mets:mptr/@xlink:href",
                                        doc);
        ...
        public void parseEadFile(File zeout) throws IOException, InvalidMetadataException {
                try {
                        ...
                        if (id == null)
                                throw new InvalidMetadataException("The ead must have a unitid");
                        if (id.isEmpty())
                                throw new InvalidMetadataException("The ead must not have an empty unitid");
                        setInstitutionalSenderId(id);
                        setItemIdentifer(id);
                                ...
                                if (bagCsum == null) {
                                        ...
                                        for (String tkey : checksums.keySet()) {
                                                if (checksums.get(tkey).equals(csum)) {
                                                        s = String.format(
                                                                        "No checksum found for path: %s; A match found in the bag with path: %s",
                                                                        name, tkey);
                                                }
                                        }
                                        errors.put(name, s);
                                } else if (!bagCsum.equals(csum)) {
                                        errors.put(name, String.format(
                                                        "Checksum Mismatch for %s [Compare File:%s; Bag:%s]", name,
                                                        csum, bagCsum));
                                }
                        }
                }
