...
public class FABagHelper {
        ...
        public String getFinalBagName() throws IncompleteSettingsException {
                ...
                if (data.newBag == null)
                        throw new IncompleteSettingsException(
                                        "Bag File must be created - call createBagFile() and writeBagFile()");
        ...
        public void setBagCountStr(String countStr) throws IncompleteSettingsException {
                ...
                if (bit == null) {
                        throw new IncompleteSettingsException(
                                        "Bag info file must be generated before the count can be set");
                }
        ...
        public static void validateBagCount(String bagNum, String bagTot) throws IncompleteSettingsException {
                ...
                if (ibagNum < 1) {
                        throw new IncompleteSettingsException(
                                        String.format("Bag Number (%d) must be 1 or larger", ibagNum));
                }
                if (!bagTot.equals("?")) {
                        try {
                                ...
                                if (ibagTot < 1) {
                                        throw new IncompleteSettingsException(
                                                        String.format("Bag Total (%d) must be 1 or larger", ibagTot));
                                }
                                if (ibagNum > ibagTot) {
                                        throw new IncompleteSettingsException(
                                                        String.format("Bag Num (%d) cannot be larger than Bag Total(%d)",
                                                                        ibagNum, ibagTot));
                                }
                        } catch (NumberFormatException e) {
                                throw new IncompleteSettingsException(
                                                String.format("Bag Total (%s) must be '?' or numeric", bagTot));
                        }
                }
